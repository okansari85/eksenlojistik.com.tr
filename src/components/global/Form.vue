<template>
  ...
  <div class="about-form shadow bg-white">
    ...
    <div class="about-form-center p-3">
      <strong>{{ $t('hr.iletisim-formu') }}</strong>
      <p class="py-3" v-html="$t('hr.iletisim-formu-p')"></p>
    </div>
    <div class="about-form-bottom px-3 py-2">
      <form action="">
        <div class="row">
          ...
          <div class="col-12">
            <textarea name="" id="" cols="2" rows="2" :placeholder="$t('hr.form-mesaj')" class="form-control" v-model="form.message"></textarea>
          </div>
        </div>
        <div class="col-12">
          <div class="d-flex justify-content-between align-items-center pt-4 pb-4">
            <input type="file" class="form-control border-0" ref="fileCv" @change="onFileChange" placeholder="CV Yükle" accept=".pdf, .doc, .docx, .zip, .rar">
            <span @click="SubmitForm" class='btn btn-main'>{{ $t('footer.gonder') }}</span>
          </div>
        </div>
